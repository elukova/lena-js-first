...
alert(a);
```
    // a === undefined, в if не зайдем. Когда загружается скрипт,
...
alert(a);
```
    // Функции `a` не будет, хоть она и не анонимная, она присваивается в переменную, в переменной
...
alert(a);
```
    // увидим функцию, т.е.в переменной a будет лежать функция.Причина в том, что все
